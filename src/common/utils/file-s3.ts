...
export async function saveFileToS3(
	...
	const safeFileName = concatStr([prefix, fileName ?? Date.now(), suffix], separator) + extension;

	const relativePath = isPathFile ? path.replace(/\\/g, '/') : join(path || '', safeFileName).replace(/\\/g, '/'); // default to just the safe filename if no path
